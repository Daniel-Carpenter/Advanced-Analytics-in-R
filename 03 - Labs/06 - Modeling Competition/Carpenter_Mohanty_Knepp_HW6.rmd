...
data.frame(Num_Numeric_Variables = ncol(df.train.base.numeric),
           Total_Observations    = nrow(df.train.base.numeric) ) %>%
  kable()
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Get summary of data frame
data.frame(Num_Factor_Variables = ncol(df.train.base),
           Total_Observations   = nrow(df.train.base) ) %>%
  kable()
```
