...
ols <- lm(totalRevenue ~ pageviews + visitNumber + timeSinceLastVisit + 
            bounces + newVisits + source + referralPath + browser + deviceCategory + 
            campaign + adContent + adwordsClickInfo.slot + adwordsClickInfo.gclId + 
            keyword + continent + subContinent + country + region + metro + city,
          data = df.train.clean)
```
...
# Best Model Output:
# totalRevenue ~ pageviews + visitNumber + timeSinceLastVisit + 
#             bounces + newVisits + source + referralPath + browser + deviceCategory + 
#             campaign + adContent + adwordsClickInfo.slot + adwordsClickInfo.gclId + 
#             keyword + continent + subContinent + country + region + metro + city,
```
...
#### Fit the Model
* Based on model testing, highest $R^2$ is around 110 number of components.  
* To ensure that the model finds the best fit, set the `ncomp` variable to 110  
* Fits data similarly to the former model.  
...
# Fit the model
pcr <- mvr(totalRevenue ~ pageviews + visitNumber + timeSinceLastVisit + 
            bounces + newVisits + source + referralPath + browser + deviceCategory + 
            campaign + adContent + adwordsClickInfo.slot + adwordsClickInfo.gclId + 
            keyword + continent + subContinent + country + region + metro + city,
          data  = df.train.clean, 
