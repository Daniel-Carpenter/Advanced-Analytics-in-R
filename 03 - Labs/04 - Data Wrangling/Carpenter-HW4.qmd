...
library(cowplot)  # multiple ggplots on one plot
```
...
```{r}
seeImputation <- function(df, df.inputed, 
                          imputationMethod) {
  ...
  meanValImpute = mean(df.inputed$y, na.rm=T)
...
df <- data.frame(y = housingData$LotFrontage, missing = missing)
```
...
df.inputed[missing,"y"]<-mean(df.inputed$y, na.rm=T)

# Compare missing vs. non missing for given imputation method
seeImputation(df, df.inputed, imputationMethod = 'Mean')
```
...
### `iii` Predicive Mean Matching Imputation
```{r}
