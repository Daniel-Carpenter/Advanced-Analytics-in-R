...
* Uses `tidyverse` piping and `dplyr` functions since use of `plyr` only specified for getting `baseball` dataset
...
* *Assumes the player name is the `id` since name not available in dataset*
...
# Get package for knitting to kable table
